...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    {
        nwrote = ::write(channel_->fd(), data, len);  // 一次性发送的长度
        if(nwrote >= 0)
        {   
    ...
    // 说明当前这一次write，并没有把数据全部发出去，剩余的数据需要保存到缓冲区当中，然后给channel
    // 注册epollout事件，poller发现tcp的发送缓冲区有空间，会通知相应的socket-channel，调用handleread
    // 也就是调用TcpConnection::handleWrite方法，把发送缓冲区的数据全部发送完
